...
kernel void generateRays(device AdvancedRay* rays [[buffer(0)]],
                         ...
                         uint2 size [[threads_per_grid]])
{
    ...
    device AdvancedRay& r = rays[rayIndex];
...
{
    uint rayIndex = coordinates.x + coordinates.y * size.x;
    device AdvancedRay& currentRay = rays[rayIndex];
    ...
    if (i.distance < 0.0f)
    {
        ...
        currentRay.radiance += currentRay.throughput * (appData.environmentColor);
    ...
    if ((currentRay.completed == 0) && (dot(material.emissive, material.emissive) > 0.0))
    {
        ...
        float weights = { appData.enableBSDFSampling ? powerHeuristic(currentRay.misPdf, samplePdf) : 0.0f };

        float weight = float(currentRay.bounces == 0) ? 1.0f : weights;
    ...
    if (appData.enableRussianRoulette) {
        if (currentRay.bounces >= 5) // 5
        {
